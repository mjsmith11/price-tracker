...
| bestbuy.com | ✅ | ✅ | `/product/*` pages are hard-blocked at the connection level (Akamai). `scrape()` works around this by re-querying search for the SKU instead of visiting the product page — see [app/adapters/bestbuy.py](app/adapters/bestbuy.py). |
